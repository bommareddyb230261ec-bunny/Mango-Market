...
class Config:
    ...
    PRODUCTION WARNING: All SECRET_KEY values must be set in environment variables.
    Do not rely on defaults in production deployment.
    """
    # CRITICAL: SECRET_KEY must come from environment for production
    # Raise error if not set to prevent weak defaults
    _secret_key = os.environ.get('SECRET_KEY')
    if not _secret_key:
        logging.error("CRITICAL: SECRET_KEY environment variable not set. Using temporary key.")
        logging.error("Set SECRET_KEY in .env or environment variables before production deployment.")
        _secret_key = 'TEMPORARY_DEVELOPMENT_KEY_CHANGE_ME'  # Only for dev
...
def create_app(test_config: dict[str, Any] | None = None) -> Flask:
    ...
    def handle_bad_request(error):
        ...
        logging.warning("Bad request: %s", error)
        return jsonify({
            'success': False,
            'error': 'Bad Request',
 'message': 'The request could not be understood by the server'
    ...
    def handle_unauthorized(error):
        ...
        logging.warning("Unauthorized access attempt: %s", error)
        return jsonify({
            'success': False,
            'error': 'Unauthorized',
    ...
    def handle_forbidden(error):
        ...
        logging.warning("Forbidden access attempt: %s", error)
        return jsonify({
            'success': False,
            'error': 'Forbidden',
    ...
    def handle_not_found(error):
        ...
        logging.debug("Resource not found: %s", error)
        return jsonify({
            'success': False,
            'error': 'Not Found',
    ...
    def handle_method_not_allowed(error):
        ...
        logging.debug("Method not allowed: %s", error)
        return jsonify({
            'success': False,
    ...
    def handle_internal_error(error):
        ...
        logging.error("Internal server error: %s", error, exc_info=True)
        return jsonify({
            'success': False,
    ...
    def handle_database_error(error):
        ...
        logging.error("Database error: %s", error, exc_info=True)
        db.session.rollback()
        return jsonify({
            'success': False,
    ...
    def handle_integrity_error(error):
        ...
        logging.error("Integrity error: %s", error, exc_info=True)
        db.session.rollback()
        return jsonify({
            'success': False,
            'error': 'Integrity Constraint Violation',
            'message': 'The data conflicts with existing records. Please check your input.'
        }), 409
