...
# Retrieve host password from environment variable (REQUIRED for production)
# Default only provided for development - MUST be set in .env for production
HOST_PASSWORD_HASH = os.getenv('HOST_PASSWORD_HASH')
HOST_PASSWORD_PLAIN = os.getenv('HOST_PASSWORD')  # Fallback for .env files with plaintext

# Log security warning if password is not properly configured
if not HOST_PASSWORD_HASH and not HOST_PASSWORD_PLAIN:
    logging.error("CRITICAL: HOST_PASSWORD not configured. Set HOST_PASSWORD_HASH or HOST_PASSWORD in .env")
    logging.error("For security: Use HOST_PASSWORD_HASH with a pre-hashed password instead of plaintext")
...
def verify_host_password():
    ...
    try:
        ...
        if not password:
            logging.warning("Host password verification attempt with empty password")
            return jsonify({
                'success': False,
        ...
        # Verify against hash (preferred) or plaintext fallback
        auth_success = False
        
        if HOST_PASSWORD_HASH:
            # Securely compare against hash
            auth_success = check_password_hash(HOST_PASSWORD_HASH, password)
        elif HOST_PASSWORD_PLAIN:
            # Fallback to plaintext comparison (less secure, only for development)
            auth_success = (password == HOST_PASSWORD_PLAIN)
            logging.warning("Host using plaintext password verification. Recommend using HOST_PASSWORD_HASH instead.")
        else:
            logging.error("Host password not configured in environment variables")
            return jsonify({
                'success': False,
        ...
        if auth_success:
            logging.info("Host password verified successfully")
            return jsonify({
                'success': True,
            ...
            logging.warning("Failed host password authentication attempt")
            return jsonify({
                'success': False,
        ...
        logging.exception("Error during host password verification: %s", e)
        return jsonify({
            'success': False,
...
def verify_host():
    """
    Verify host password for access to verification panel (alternate endpoint)
    ...
    try:
        ...
        # Verify password using same logic as above
        auth_success = False
        ...
        logging.exception("Error during host verification: %s", e)
        return jsonify({
            'success': False,
